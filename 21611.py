...
for blizzard_d, blizzard_s in move_list:
    # blizzard 마법 - 블리자드의 방향, 거리에 있는 구슬 폭발
    current_r = (N//2)+1
    current_c = (N//2)+1
    for _ in range(blizzard_s):
        current_r += bli_rc[blizzard_d][0]
        current_c += bli_rc[blizzard_d][1]
        blocks[current_r][current_c] = 0

    one_list = list()
    # blizzard 마법 이후 일렬 리스트로 붙이기
    make_one_list()

    # blizzard 마법 이후 블록 재배치
    make_blocks()
...
# ref) https://velog.io/@attractive_minki/Python-%EB%B0%B1%EC%A4%80-21611-%EB%A7%88%EB%B2%95%EC%82%AC-%EC%83%81%EC%96%B4%EC%99%80-%EB%B8%94%EB%A6%AC%EC%9E%90%EB%93%9C